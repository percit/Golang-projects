...
func main() {
	...
	routes := Routes{ //this is so every route has access to database insance
		Database: database,
	}
...
func (r *Routes) login(c *gin.Context) {
	if c.Request.FormValue("password") == appPassword { //jesli haslo jest takie jak ustawione w session, to zadziala
